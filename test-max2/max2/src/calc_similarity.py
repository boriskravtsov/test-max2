...
from max2.src.draw_similarity_map import draw_similarity_map
    ...
    for j in range(1, n_image_peaks):
        ...
        for i in range(1, n_templ_peaks):
            ...
            if cfg_max2.debug_mode:
                ...
                count += 1
                if count < 15:
                    map_name = '_similarity_' + str(count)
                    draw_magn(map_name, similarity_map_current)
                """
            else:
...
def calc_similarity_cont_map(
    ...
    if nonzero_count == 0:
        ...
        similarity = 1.0 / (1.0 + cfg_max2.param_similarity * average)
